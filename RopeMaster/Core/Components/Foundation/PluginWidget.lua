...
return function(componentName, createWidgetFunc)
	...
	function PluginWidget:init(props)
		...
		if props.BindToCloseCallback then
			self.widget:BindToClose(props.BindToCloseCallback)
		else
			self.widget:BindToClose(
				function()
					self.widget.Enabled = false
				end
			)
		end
		self.widget:GetPropertyChangedSignal("Enabled"):connect(
			function(...)
		...
		self.widget.AncestryChanged:connect(
			function(...)
